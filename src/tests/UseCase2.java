...
class UseCase2 {
	...
	void test() throws IOException {
		...
		//Criar os Voluntarios e Ajudas pelo Handler (criar os Items primeiro para poder testas os diferentes sorters)
		...
		for (int i = 0; i < nomesIndiv.length; i++) {
			...
			Ajuda[] ajudas = TestingTools.setupMigranteIndividual(handler, nomesIndiv[i], telesIndiv[i], regioesIndiv[i]);
			...
			switch (i) {
			case 0: { // Ivan
				testCase(ajudas, ajudaCatalog, migrante, handler, new int[]{0,1,2,3,4}, new int[]{0,3}, out, false);
				break;
			}
			case 1: { // Oleksiy
		...
		for (int i = 0; i < nomesFam.length; i++) {
			PedeAjudaHandler handler = new PedeAjudaHandler();
			Ajuda[] ajudas = TestingTools.setupMigranteFamilia(handler, nomesFam[i], telesFam[i], familias[i], regioesFam[i]);
	...
	private void testCase(Ajuda[] ajudas, AjudaCatalog ajudaCatalog, Migrante migrante, PedeAjudaHandler handler, int[] ajudasEsperadas, int[] ajudasEscolhidas, ByteArrayOutputStream out, boolean ativarNotificacao) throws IOException {
